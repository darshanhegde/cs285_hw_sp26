...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        total_return = np.sum(discounts * np.array(rewards))
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            # without a baseline, the advantage is just the Q-value itself
            ...
            if self.gae_lambda is None:
                # with a baseline but no GAE, the advantage is the Q-value minus the baseline value
                ...
                for i in reversed(range(batch_size)):
                    ...
                    # terminals[i] is 1 if the state is the last in its trajectory, in which case we neither bootstrap
                    # from V(s_{i+1}) nor carry over A_{i+1}, since index i+1 belongs to the next trajectory.
                    not_done = 1 - terminals[i]
                    delta = rewards[i] + self.gamma * values[i + 1] * not_done - values[i]
                    advantages[i] = (
                        delta + self.gamma * self.gae_lambda * not_done * advantages[i + 1]
                    )
